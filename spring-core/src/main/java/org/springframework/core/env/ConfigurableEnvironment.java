...
/**
 * 该Environment接口主要主要提供配置profile和properties的能力。
 * ConfigurableEnvironment继承了ConfigurablePropertyResolver接口，该接口是前文中的PropertyResolver的配置实现：
 * 该配置环境接口提供了获取多属性源的接口：
 * MutablePropertySources getPropertySources()，通过该接口获取易变的多属性源可以达到操作Environment中的属性源的操作：
 * addBefore/addFirst/addAfter/AddLaster/remove等等操作属性源的接口。
 */
